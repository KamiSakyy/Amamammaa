...
public final class CalendarScreen extends FrameLayout {
    ...
    private final Activity activity;private final RecyclerView recycler;private final LinearLayoutManager layout;private final CalendarAdapter adapter;private final SwipeRefreshLayout pull;
    ...
    private void load(boolean force){int gen=++generation;if(loadFuture!=null)loadFuture.cancel(true);prepareDays();refreshing=true;if(pull!=null)pull.setRefreshing(true);state="Календарь";rebuildVisible(true);adapter.publish();loadFuture=YoruApp.app().discovery.submit(()->{ArrayList<ApiRepository.AiringItem> cached=new ArrayList<>();boolean fresh=false;try{cached=ApiRepository.airingFromJson(YoruApp.app().cache.schedule(6*60*60*1000L));if(cached.isEmpty())cached=ApiRepository.airingFromJson(readLegacyCache());fresh=!cached.isEmpty()&&System.currentTimeMillis()-YoruApp.app().store.calendarCacheAt()<25*60*1000L;}catch(Exception ignored){}final ArrayList<ApiRepository.AiringItem> cacheSnapshot=new ArrayList<>(cached);if(!cacheSnapshot.isEmpty())YoruApp.app().main.post(()->{if(alive()&&gen==generation&&items.isEmpty())setItems(cacheSnapshot,true);});if(fresh&&!force){YoruApp.app().main.post(()->{if(alive()&&gen==generation){stopPull();if(!cacheSnapshot.isEmpty())setItems(cacheSnapshot,true);}});return;}try{ArrayList<ApiRepository.AiringItem> loaded=YoruApp.app().api.airingSchedule(DAYS,YoruApp.app().store.favorites());JSONArray json=ApiRepository.airingJson(loaded);YoruApp.app().cache.schedule(json);YoruApp.app().store.calendarCache(json);YoruApp.app().calendarTodayCount=ApiRepository.todayCount(loaded);YoruApp.app().main.post(()->{if(!alive()||gen!=generation)return;stopPull();setItems(loaded,true);});}catch(Exception e){YoruApp.app().main.post(()->{if(!alive()||gen!=generation)return;stopPull();if(items.isEmpty()&&!cacheSnapshot.isEmpty())setItems(cacheSnapshot,true);else{state="Календарь";rebuildVisible(false);adapter.publish();}});}});if(loadFuture.isCancelled()){stopPull();Ui.toast(activity,"Очередь занята. Нажмите обновление позже.");}}
    ...
    private TimeZone zone(){return TimeZone.getTimeZone(msk()?"Europe/Moscow":TimeZone.getDefault().getID());}
    private long todayStart(){Calendar c=Calendar.getInstance(zone());c.set(Calendar.HOUR_OF_DAY,0);c.set(Calendar.MINUTE,0);c.set(Calendar.SECOND,0);c.set(Calendar.MILLISECOND,0);return c.getTimeInMillis();}
    private long startOfDay(long t){Calendar c=Calendar.getInstance(zone());c.setTimeInMillis(t);c.set(Calendar.HOUR_OF_DAY,0);c.set(Calendar.MINUTE,0);c.set(Calendar.SECOND,0);c.set(Calendar.MILLISECOND,0);return c.getTimeInMillis();}
    private boolean msk(){try{return YoruApp.app().store.mskTime();}catch(Exception e){return true;}}
    private SimpleDateFormat fmt(String pattern){SimpleDateFormat f=new SimpleDateFormat(pattern,ru);if(msk())f.setTimeZone(TimeZone.getTimeZone("Europe/Moscow"));return f;}
    private String time(long t){return fmt("HH:mm").format(new Date(t))+(msk()?" МСК":"");}
        ...
        CalendarHolder(int type) {
            ...
            if(type==1) {
                ...
                View download=Ui.iconButton(activity,"download","Скачать после выхода",()->{
                    if(item==null)return;
                    ScheduledDownloads.choose(activity,item.anime,item.episode,item.time,fmt("dd.MM.yyyy HH:mm").format(new Date(item.time))+" · "+item.precision);
