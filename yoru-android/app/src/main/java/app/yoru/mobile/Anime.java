...
public final class Anime {
    ...
    public String episodesLine(){int aired=aired();int total=episodes;if(!ongoing()&&total>0&&aired>0&&aired<total)total=aired;if(total>0&&aired>0&&aired<total)return "Вышло "+aired+" из "+total;if(total>0)return total+(total==1?" серия":total<5?" серии":" серий");if(aired>0)return "Вышло "+aired;return "";}
