...
- В коде не оставлять логирование (Log.*, println, System.out) — чистый продакшен-код.
- Дизайн улучшать, а не перестраивать: фирменная палитра (0xff0d0b12, 0xff1c1724, 0xff15111c, 0xffc8a7ff, 0xfff7f0ff, 0xffa99bb8, 0xff352a43) не меняется.
...
## Обновление 4.20.0 — 2026-09-14

- Большой релиз от базы 4.19.1, versionCode 71, compileSdk/targetSdk 36 (возвращено к 4.19.1; промежуточная попытка с 37 отменена владельцем). Сборка без минификации/усушки ресурсов по указанию владельца. Палитра и общая структура дизайна сохранены; изменения только над существующим дизайном.
- Сеть переведена на OkHttp 5.x (Net + VideoResolver + DownloadSizes + ImageLoader + YummyParser + YoruShield + EpisodeUpdateReceiver); пул соединений и параллелизм выше, приоритет — задачам активного экрана (focusNow очищает фоновую очередь при открытии деталей/плеера).
- Карточки: оценка Shikimori вместо бейджа озвучки, без полосы прогресса; кэш-обложки показываются без мигания заглушки.
- Главная: бесконечный топ (подгрузка по 6), SwipeRefreshLayout, ежедневный «Выбор дня», тихое автообновление раз в час, «Рекомендации».
- Детали: блок «Смотреть» под описанием (сезон/озвучка/серия), озвучки по популярности, кликабельные жанры открывают каталог с фильтром.
- Плеер: скриншоты (TextureView), блокировка касаний, точные свайпы громкости/яркости, только реально доступные разрешения, подписи 2K/4K без «Quality+».
- Календарь: МСК-подписи и настройка местного времени, устойчивость к переносам дат, swipe-refresh.
- Загрузки: точный размер до старта (для HLS — оценка по сегментам), сохранение в выбранную папку через системный выбор (DocumentFile), аккуратная иконка удаления.
- Внешний вид: обои с предпросмотром и масштабом, прозрачный режим со слайдером и предпросмотром.
- Исправлено число серий у завершённых тайтлов; изображения по умолчанию в максимальном качестве, при экономии трафика — сжатие.
- Подпись: исходный ключ 4.19.1 в репозитории отсутствовал (не входил в source ZIP), поэтому создан новый релизный ключ `yoru-android/owner-signing` (PKCS12). Ожидаемый SHA-256 подписи обновлён в `certificate-sha256.txt`. Установка 4.20.0 поверх 4.19.1 невозможна — требуется удаление старой версии.
- Сборка: `.github/workflows/build-apk.yml` публикует `YORU-4.20.0-release.apk` в текущую ветку, проверяет подпись и лимит 3 МБ.
- Удалён `assets/posters` (локальные заглушки не использовались) для уменьшения APK.
