# YORU 4.20.0 — большой релиз: дизайн, скорость, плеер, загрузки

Обновление от базы 4.19.1, versionCode 71. Дизайн сохранён и улучшен, палитра без изменений.

- Карточки: вместо бейджа озвучки — оценка Shikimori (★ 9.8), убрана полоса прогресса; кэш-обложки появляются мгновенно, без мигания заглушки.
- Главная: бесконечный топ с подгрузкой по 6 карточек, плавное обновление свайпом вниз, подборка дня меняется ежедневно, тихое автообновление раз в час; раздел переименован в «Рекомендации».
- Детали: блок «Смотреть» сразу под описанием — выбор сезона (связанные ТВ-сезоны), озвучки по популярности и серии, кнопка «Начать просмотр»; жанры кликабельны и открывают каталог с фильтром.
- Плеер: скриншот кадра в галерею, блокировка касаний, более точные свайпы громкости/яркости, список качеств только из реально доступных (144p/240p показываются, только если есть), честные подписи 2K/4K вместо «Quality+».
- Сеть полностью на OkHttp 5.x (пул соединений, параллельные запросы, приоритет текущему экрану), эпизоды подтягиваются ещё во время открытия карточки.
- Фильтры: все жанры Shikimori и источников, закрытие красивой кнопкой-крестиком, убраны подсказки и «Отменить»; убран ярлык «История» в каталоге.
- Календарь: время МСК с подписью (и настройка местного времени), устойчив к переносам дат, обновление свайпом.
- Загрузки: точный реальный размер до скачивания (для HLS — по сегментам), сохранение в выбранную пользователем папку через системный выбор, аккуратная иконка удаления.
- Внешний вид: свои обои из галереи/Google Фото с предпросмотром и масштабом, прозрачный режим со слайдером и предпросмотром.
- Исправлены несоответствия числа серий у завершённых тайтлов; изображения по умолчанию в максимальном качестве, экономия трафика сжимает.
- Окно настроек «Разрешение» (без «Quality+»), минималистичная иконка настроек.
- compileSdk/targetSdk 36, minSdk 26. Сборка без минификации (как в исходной 4.19.1): `.github/workflows/build-apk.yml`, APK публикуется в текущую ветку.

Внимание: релиз подписан новым сертификатом (исходный ключ 4.19.1 в репозитории отсутствует). Перед установкой 4.20.0 удалите прежнюю версию приложения; данные хранятся во внутренней памяти и при чистой установке не переносятся.

[Release APK](apk-output/YORU-4.20.0-release.apk) · [Debug APK](apk-output/YORU-4.20.0-debug.apk)
